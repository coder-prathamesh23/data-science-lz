...
data "terraform_remote_state" "shared_storage" {
  count   = var.shared_storage_remote_state.enabled ? 1 : 0
  backend = "azurerm"

  config = {
    resource_group_name  = var.shared_storage_remote_state.resource_group_name
    storage_account_name = var.shared_storage_remote_state.storage_account_name
    container_name       = var.shared_storage_remote_state.container_name
    key                  = var.shared_storage_remote_state.key
    use_azuread_auth     = try(var.shared_storage_remote_state.use_azuread_auth, true)
  }
}
